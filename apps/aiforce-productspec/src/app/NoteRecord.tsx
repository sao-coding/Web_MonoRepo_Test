...
import { CheckIcon, Minimize2, PencilIcon, Trash2 } from 'lucide-react'
...
const NoteRecord: React.FC<NoteViewProps> = ({ onToggleViewNote, selectedRecord }) => {
  ...
  const fetchNotes = async () => {
    try {
      const res = await fetch(
        `${getAppConfig().NEXT_PUBLIC_PATENT_SERVICE_API_URL}/api/aicity/productspec/notes/${selectedRecord}/references/detail`,
        {
          method: 'GET'
        }
      )
  ...
  const updateTitle = async () => {
    ...
    try {
      const res = await fetch(
        `${getAppConfig().NEXT_PUBLIC_PATENT_SERVICE_API_URL}/api/aicity/productspec/notes/${selectedRecord}`,
        {
          method: 'PATCH',
          headers: {
          ...
          <div className='flex h-14 items-center justify-between border-b px-4'>
            {isEditingTitle === note.noteInfo[0]?.noteId ? (
              ...
              >
                ...
                />
                <Button size='sm' onClick={() => updateTitle()} className='h-8 px-2'>
              ...
              <>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <Button variant='ghost' className='size-7 p-0' onClick={onToggleViewNote}>
                    <Minimize2 />
                  </Button>
                </div>
              </>
          ...
          <div className='flex-1 overflow-y-auto p-4 sm:p-6'>
            <ReactMarkdown
              ...
              components={{
                table: ({ ...props }) => (
                  <div
                    className='my-4 max-w-full overflow-x-auto rounded-lg border'
                    style={{ maxWidth: '100%' }}
                  >
                ...
                th: ({ ...props }) => (
                  <th
                    className='whitespace-nowrap border bg-gray-100 p-2 text-left font-bold dark:border-zinc-700 dark:bg-zinc-800'
                    {...props}
                ...
                td: ({ ...props }) => (
                  <td
                    className='border p-2 dark:border-zinc-700'
                    {...props}
                    style={{
                      minWidth: '60px',
                      maxWidth: '200px',
                ...
                code: ({ ...props }) => (
                  <code
                    className='rounded-sm bg-gray-100 px-1 py-0.5 text-sm dark:bg-zinc-800'
                    {...props}
                  />
                ),
                blockquote: ({ ...props }) => (
                  <blockquote
                    className='my-2 border-l-4 border-gray-300 pl-4 italic dark:border-zinc-600'
                    {...props}
                  />
                )
              }}
            ...
            <div className='mt-2'>
              {note.isWeb ? (
                ...
                <Button
                  ...
                  variant='outline'
                  onClick={() => {
                    handleReferencesClick(note.recordDetailId)
                  }}
                  style={{ borderRadius: '25px' }}
                >
                  參考資料
