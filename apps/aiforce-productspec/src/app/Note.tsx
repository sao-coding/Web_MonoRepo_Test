...
const NoteComponent: React.FC<NoteProps> = ({ userId }) => {
  ...
  const fetchNotes = async () => {
    try {
      const res = await fetch(
        `${getAppConfig().NEXT_PUBLIC_PATENT_SERVICE_API_URL}/api/aicity/productspec/notes?userId=${userId}`,
        {
          method: 'GET'
        }
      )
  ...
  return (
    <>
      {isNoteRecord ? (
        <NoteRecord onToggleViewNote={toggleViewNote} selectedRecord={selectedRecord} />
      ) : (
        <>
          {/* 記事內容區 */}
