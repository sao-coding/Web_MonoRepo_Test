...
export interface UseInstantOptions {
  /** API 基礎 URL */
  apiBaseUrl?: string
  /** 使用者 ID */
...
export interface UseInstantReturn {
  /** 對話記錄列表 */
  instants: InstantItem[]
  /** 設定對話記錄 */
  setInstants: React.Dispatch<React.SetStateAction<InstantItem[]>>
  /** 歷史記錄列表 */
  records: RecordItem[]
  /** 設定歷史記錄 */
  ...
  /** 開始新對話 */
  startNewInstant: () => void
  /** 新增對話 */
  addInstant: (instant: InstantItem) => void
  /** 更新最後一則對話的答案 */
...
export function useInstant({
  apiBaseUrl,
  userId
}: UseInstantOptions = {}): UseInstantReturn {
    ...
    async (recordId: number) => {
      try {
        ...
        if (details?.qaPairs && details.qaPairs.length > 0) {
          const loadedInstants: InstantItem[] = details.qaPairs.map((detail) => ({
          ...
          setInstants(loadedInstants)
          if (loadedInstants.length > 0) {
            const lastAnswer = loadedInstants[loadedInstants.length - 1].answer
  ...
  const startNewInstant = useCallback(() => {
    setActiveRecordId(null)
    setInstants([])
    setMessage('')
  }, [])

  const addInstant = useCallback((instant: InstantItem) => {
    setInstants((prev) => [...prev, instant])
  }, [])
