...
const AppCard = (
  ...
  const fetchApp = async () => {
    setIsLoading(true)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_AIforce_API_URL}/api/AIForce/Systems?userId=${user?.userId}&deptId=${user?.deptId}&category=${categoryId}&lang=${langCode}`, {
        method: 'GET',
      })
      if (res.ok) {
  ...
  const handleStar = async (sysId: number) => {
    try {
      await fetch(`${process.env.NEXT_PUBLIC_AIforce_API_URL}/api/AIForce/FavoriteToggle`, {
        method: 'POST',
