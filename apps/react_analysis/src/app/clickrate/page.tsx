...
export default function AnalysisPage() {
  ...
  const processData = (data: any[]) => {
    ...
    if (filters.FunctionMode === 'SystemCentric') {
      ...
      const systemGroups: { [key: string]: { count: number } } = {}
      ...
      // 計算總數
      const totalCount = Object.values(systemGroups).reduce((sum, group) => sum + group.count, 0)

      // 轉換為最終格式並計算百分比
      return Object.entries(systemGroups).map(([system, data]) => {
        const percentage = totalCount > 0
          ? (data.count / totalCount) * 100
          : 0
        return {
          name: system.length > 12
            ? `${system.substring(0, 12)}...`
            : system,
          value: data.count,
      ...
      const buGroups: { [key: string]: { count: number } } = {}
      ...
      // 計算總數
      const totalCount = Object.values(buGroups).reduce((sum, group) => sum + group.count, 0)

      // 轉換為最終格式並計算百分比
      return Object.entries(buGroups).map(([bu, data]) => {
        const percentage = totalCount > 0
          ? (data.count / totalCount) * 100
          : 0
        return {
  ...
  const fetchData = async () => {
    try {
      ...
      const params = new URLSearchParams({
        ...
        FilterUnits: filters.FilterUnits,
          ...
          <div className="flex-1">
            ...
            {chartData.length > 0 && (
              <div className="mt-6 bg-white rounded-lg shadow p-6">
                <h4 className="text-md font-semibold text-gray-800 mb-3">詳細數據</h4>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-200">
                        ...
                        <th className="text-center py-2 px-3">數量次數</th>
                        <th className="text-center py-2 px-3">占比</th>
                      </tr>
                    </thead>
                    <tbody>
                      {chartData.map((item, index) => (
                        <tr key={index} className="border-b border-gray-100">
                          ...
                          <td className="text-center py-2 px-3">{item.value.toLocaleString()}</td>
                          <td className="text-center py-2 px-3">{item.percent}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
